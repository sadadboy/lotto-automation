...
# -*- coding: utf-8 -*-
"""
완전 자동화 로또 구매 시스템 - 도커 최적화 버전
사용자 입력 없이 완전 자동화된 실행
"""
...
class AutomatedLottoBuyer:
    ...
    def login(self):
        ...
        try:
            ...
            # ID 입력
            id_input = self.driver.find_element(By.ID, "userId")
            id_input.clear()
    ...
    # 여기에 나머지 구매 로직 메서드들을 그대로 포함 (기존과 동일)
...
def main():
    """메인 함수 - 완전 자동화"""
    parser = argparse.ArgumentParser(description='자동화 로또 구매 시스템')
