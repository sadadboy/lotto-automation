...
# -*- coding: utf-8 -*-
"""
TAB + ENTER 방식 완전 자동화 로또 구매 시스템 - 최종 완성 버전
HTTP 세션 정리 및 GitHub/Docker 배포 준비 완료
"""
...
class ProductionLottoBuyer:
    """프로덕션 환경용 완전 자동화 로또 구매 클래스"""
    ...
    def cleanup(self):
        ...
        try:
            ...
            # 알림 매니저 정리 (HTTP 세션 닫기)
            if self.notification_manager:
                try:
                    # 비동기 정리를 안전하게 실행
                    import asyncio
                    
                    # 현재 스레드에서 새로운 이벤트 루프 생성
                    try:
                        loop = asyncio.new_event_loop()
                        asyncio.set_event_loop(loop)
                        loop.run_until_complete(self.notification_manager.cleanup())
                    except Exception as e:
                        print(f"⚠️ 알림 매니저 정리 실패: {e}")
                    finally:
                        try:
                            loop.close()
                        except:
                    ...
                    print(f"⚠️ 정리 과정 오류: {e}")
                    
                self.notification_manager = None
                
        except Exception as e:
            print(f"⚠️ cleanup 오류: {e}")
    
    def _signal_handler(self, signum, frame):
        """신호 핸들러 - 안전한 종료"""
        print(f"📡 신호 수신: {signum}, 안전하게 종료합니다...")
        self.cleanup()
        sys.exit(0)
    ...
    def setup_logging(self):
        ...
        # 파일 핸들러 (항상 추가)
        try:
            file_handler = logging.FileHandler('logs/lotto_auto_buyer.log', encoding='utf-8')
...
def main():
    """메인 함수"""
    parser = argparse.ArgumentParser(description='프로덕션 TAB + ENTER 방식 자동화 로또 구매 시스템')
